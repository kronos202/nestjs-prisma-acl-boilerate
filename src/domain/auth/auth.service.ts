...
import { HttpMethod, Session, User } from '@prisma/client';
...
export class AuthService {
    ...
    const user = await this.databaseService.user.create({
      data: {
        ...
        UserRole: {
          create: {
            role: {
              connect: { name: 'user' }, // Gán role mặc định là 'user'
            },
          },
        },
      },
  ...
  private async getTokensData(data: {
    id: User['id'];
    roles: string[];
    permissions: { method: HttpMethod; path: string }[];
    sessionId: Session['id'];
    hash: Session['hash'];
  }) {
  ...
  private async getUserPermissions(userId: string) {
    const userWithPermissions = await this.databaseService.user.findUnique({
      where: { id: userId },
      include: {
        UserRole: {
          include: {
    ...
    return userWithPermissions.UserRole.flatMap((userRole) =>
      userRole.role.RolePermission.map((rp) => rp.permission),
    );
  }
  async test(hash: string) {
    const ver = this.jwtService.verifyAsync(hash, {
      secret: 'somethingmawenestAUTH_JWT_SECRETAUTH_REFRESH_SECRET',
    });
    return ver;
  }
}
